...
pg~append(f~htstyle("h2",,"red","p","lightblue","white")
...
::method httitle
expose r
use strict arg title=.nil
if title=.nil then title="Untitled Page"
...
smb=.mutablebuffer~new("<style>")
  loop i=1 to args() by 3
    select -- case of colors...
    when arg(i+1)=".css" & arg(i+2)=".css" then nop
    when arg(i+1)=".css" then smb~append("color:" arg(i+1))
    when arg(i+2)=".css" then smb~append("background-color:" arg(i+1))
    otherwise smb~append("background-color:" arg(i+1) "color:" arg(i+2))
    end
  smb~insert(arg(i) "{")
  smb~append("}")
  end
